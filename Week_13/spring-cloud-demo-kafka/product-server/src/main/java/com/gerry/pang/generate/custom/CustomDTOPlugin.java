...
public class CustomDTOPlugin extends PluginAdapter {
    ...
    public void initialized(IntrospectedTable introspectedTable) {
        ...
        if (ignoreFields) {
        	this.ignoreFieldSet = Arrays.stream(properties.getProperty("ignoreFields").split(",")).collect(Collectors.toSet());
        } else {
        	this.ignoreFieldSet = new HashSet<>(16);
        }
    ...
    private void fieldGenerated(TopLevelClass topLevelClass, IntrospectedColumn introspectedColumn) {
        ...
        if (ignoreFields && ignoreFieldSet.contains(field.getName())) {
            return ;
        }
